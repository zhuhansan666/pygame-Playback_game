    ...
    def mainloop(self, skip_head: bool = False):
        if skip_head is not True:
            ...
            while self.in_head:
                ...
                self.tools.draw_text(self.screen, "按下\"Q\"以输入您的qq号",
                                     (10, 240), 35, stop_time=0)
                self.tools.draw_text(self.screen, "注意：您可以按下\"E\"手动增加弹球数量",
        ...
        if online:
            ...
            if type(res) == dict:
                self.tools.draw_text(self.screen, "个人历史最高成绩: {} 分".format(res["personal_tallest"]), (150, 310), 50,
                                     stop_time=0)
                self.tools.draw_text(self.screen, "个人排名: {}".format(res["rank"]), (150, 370), 50, stop_time=0)
        else:
            self.tools.draw_text(self.screen, "您没有登陆！", (150, 310), 50, color="red", stop_time=0)
            self.tools.draw_text(self.screen, "登陆即可成绩云同步并保存最高成绩 (按\"Q\"登陆)", (150, 370), 50, color="green", stop_time=0)
